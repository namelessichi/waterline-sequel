...
Sequel.prototype.count = function count(currentTable, queryObject) {
    ...
    // if we have `queryObject.where.joins` - make joins!
    // TODO bug count error if populate without `where`/filter for refs
    if (queryObject.where && queryObject.where.joins) {

        queryObject.instructions = {};

        _.forEach(queryObject.where.joins, function (join) {
            queryObject.instructions[join.parentKey] = {
                strategy: {
                    strategy: 1
                },
                meta: null,
        ...
        delete queryObject.where.joins;
    }
    ...
    this.queries[0] += '(SELECT ' + tableName + '.* FROM ' + tableName + ' ' + whereObject.query + ') AS ' + alias;
    this.values[0] = whereObject.values;
